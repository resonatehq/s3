...
-- `transHolds` less the property of a finding below, one each.
pred transHoldsButNoDeadDispatch [now : Int] {
...
-- A finding. `preserved_no_dead_dispatch` asks that a task entering
-- `pending` have a promise still pending at `now`; but the schedule
-- trigger fires the occurrences due since its last run at their own
-- instants, and an occurrence far enough in the past creates a promise
-- whose timeout has already passed by `now`, with a pending task that
-- will never be dispatched. The trigger satisfies every other transition
-- property; the property alone fails, and the run shows how.
check processSchedule_transitions {
  all mat : Bool, now : Int, req : ScheduleTimeoutReq |
    stateHolds[now] and well_formed_config_retry_positive and processSchedule[mat, now, req]
      implies (transHoldsButNoDeadDispatch[now] and internalWellFormed[now])
} for 4 but 5 Int, 2 seq, 2 steps, 2 Schedule

check processSchedule_no_dead_dispatch {
  all mat : Bool, now : Int, req : ScheduleTimeoutReq |
    stateHolds[now] and processSchedule[mat, now, req] implies preserved_no_dead_dispatch[now]
} for 4 but 5 Int, 2 seq, 2 steps, 2 Schedule

run scheduleFiresLate {
  some mat : Bool, now : Int, req : ScheduleTimeoutReq |
    stateHolds[now] and processSchedule[mat, now, req] and
    some o : State.objects' | no object[o.id] and o.task.state = TaskPending and
      o.promise.state = Pending and o.promise.timeoutAt <= now
} for 4 but 5 Int, 2 seq, 2 steps, 2 Schedule
