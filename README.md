...
A document is stored as a list of parts. A handler appends to the document, `Origin.set` never overwrites, and `Origin.current` folds the log into the view, the latest version of every object. The configuration `Config.adds` says how many parts a document may hold before a request writes it whole: with zero adds every write is a whole put, as on S3; with `n` adds a request appends one part, as on S3 Express One Zone, and the request that would exceed `n` writes the compacted document.
