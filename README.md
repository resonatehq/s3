...
| `src/impl/cache.lean` | The machine with a read cache next to the bucket. `Cached H` is a bucket and a list of entries by origin, each the document and the etag the put was acknowledged with; `runCached` reads the origin from the cache when it is there and conditions the put on the stored etag, otherwise reads from the bucket and conditions on the hash of what it read, and once the put is accepted remembers the document with the etag of the blob it wrote. Handlers and `handle` are untouched. `Sound` says every entry is the blob stored at its path and its etag is that blob's hash; it holds initially, every step preserves it, and under it the cached step equals the uncached step on the bucket. `refinesCached` is `refines` for the cached machine. That theorem never meets a stale entry, because the model has one writer; the dichotomy lemmas do. For any cache whose entries carry their own document's etag (`Tagged`), `runCached_dichotomy` says: if the stored etag matches the blob in the bucket, the step is the uncached step and the put is accepted; if it does not, the put is refused, no origin blob and no message changes, and the entry is forgotten. `stepCached_dichotomy` lifts this to a request step, where a refusal shows as a stutter. |
