...
`model/store.lean` is the second experiment: the bucket and the servers as separate actors. A request is three S3 calls, each its own step, interleaved with the other servers': a GET that snapshots the origin, a PUT of the timers the handler arms, and the conditional PUT of the document, refused if the origin's content changed since the GET, as an etag is a hash of the content. The handlers are `promiseCreate` and `promiseSettle` with the timeout sweep and the timer event.

With any number of servers, `pending_armed` holds and is proved inductive by SMT: every pending awaitable promise has its timer, so no timeout is missed, whatever the interleaving. Two properties of the single-writer machine do not survive. A refused write leaves the timers of the refused attempt in the bucket, `stray_timer` is the trace, so `armed` fails: a timer can exist for an object that was never written, or for an internal promise, or with a deadline that is not its object's timeout. Nothing deletes such a timer, since `Concrete.step` only sweeps on a timer and the sweep deletes the timers of the objects it settles, so it fires at every instant from its deadline on. The documents are unaffected; the cost is a timer that never stops firing. The Lean proofs do not cover this: `armed` is stated for the single-writer trace, and `runCached_any` relates the documents and the outbox to the uncached machine, not the timers. Two servers over two objects are model checked exhaustively, 284,370 states, once servers clear their snapshot when they go idle; before that, the stale copies on idle servers put even one object beyond reach. Three servers over three instants are covered by `#simulate`.
