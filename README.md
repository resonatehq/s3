...
| `src/impl/refinement.lean` | The theorem, stated. An `Observation` is a request, its response and the instant; `observed tr n` is what the first `n` frames of a trace show, and `nth tr k o` says `o` is the k-th thing shown. `abstract` maps a bucket to an abstract state: the objects of every origin blob, no schedules, the outbox as is. `Equiv` relates abstract states by lookup, `Inv` is the bucket invariant (an origin blob holds only its own objects, ids unique), and `step_sim` is the one-step simulation: a concrete step is a list of abstract events at the same instant that preserves the relation and shows the same observation. `refines`: every valid concrete trace from the empty bucket has a valid abstract trace from the empty state with the same k-th observation for every k. Proof pending. |
