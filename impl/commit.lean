...
/-!  # The commit — what performing a transaction does to the world

The layer between the kernel's decision and the refinement: the closed
form of a transaction's effects (`transact_apply`), what each kind of
effect does to the bucket, and the two invariants of the store that the
CAS argument rests on.

  `SInv`     every stored version is below the store's counter — the
             fact that makes versions fresh, so a version seen once is
             never seen again at a different document.
  `TxnInv`   an in-flight snapshot is either at the version still
             stored, and then it IS the stored document, or at a version
             that has moved. Preserved by every effect: a document write
             moves the version (`TxnInv.of_put`), everything else leaves
             document keys alone (`TxnInv.of_get_eq`).

`snapshot_current` is the CAS, as a theorem: if the condition a
transaction's write is made under holds of the current version, the
document the transaction decided against is the current document.
`runC_transact` is the commit in closed form — arm, one conditional
write, and the rest only if it landed. -/
...
/-- The document an origin currently holds — empty if none was written. -/
...
/-- What a transaction decides and emits, in closed form. -/
...
/-! ### Effects that are not the document write -/

def _root_.Impl.Effect.isPutDoc : Impl.Effect → Bool
  | .putDoc _ => true
  | _         => false

/-- The messages a list of implementation effects hands to the wire. -/
...
/-! ### The store's invariant, and what each effect does to the world -/

/-- Every stored version is below the counter: versions are fresh. -/
...
/-- What one non-document effect does: it lands, it leaves every document
    key alone, it advances the counter or leaves it, and it sends what it
    says. -/
...
/-- A snapshot is either at the version still stored — and then it IS the
    stored document — or at a version that has moved. Either way it is
    below the counter. -/
...
/-- A document write at `o` keeps every snapshot's invariant: a snapshot
    of `o` is now at a moved version, a snapshot of anything else is
    untouched. -/
...
/-! ### One commit, performed -/

/-- The condition a transaction's write is made under holds of the current
    version exactly when the snapshot is current. -/
...
/-- The environment a transaction commits in. -/
...
/-- The world after the timer it arms first. -/
...
/-- The commit of a transaction, performed against the world: arm, then
    the one conditional write, then — if it landed — the rest. -/
