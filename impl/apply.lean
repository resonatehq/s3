...
/-!  # Effects, seen through lookups

The specification's `Effect.apply` rebuilds an object list — a cons here,
a filter there, a map over everything. The implementation never sees
that list; it sees one document, and the refinement compares the two by
LOOKUP: what does the state say about id `j`?

So this file restates every effect as a transformer of lookups
(`applyFind`), shows the restatement exact (`lookup_applyAll`), and
proves the two facts the refinement needs about a list of effects:

  * CONGRUENCE — two lookups that agree on origin `o` still agree on
    origin `o` after the same effects. No locality needed: a write at
    `id` reads only `id`, and `id` is the one being asked about.
  * FRAME — local effects (`Effect.Local o`) leave every other origin's
    lookups exactly as they were.

The outbox is a fold of the messages sent (`sendsFold`), and schedules
are untouched by local effects. Both are stated in the form the world's
`wire` evolves in, so the two sides can be compared directly. -/
...
/-- The row `.setPromise` writes is at the id written — provided what was
    found at that id was at that id, which every lookup guarantees. -/
...
/-- One effect, as a lookup transformer. -/
...
/-! ## Well-formed lookups: what you find at `j` is at `j` -/
...
/-! ## Origins of the objects written -/
...
/-! ## The outbox and the schedules -/

/-- The specification's outbox discipline, folded over a list of sends:
    one live message per key, newest first. -/
